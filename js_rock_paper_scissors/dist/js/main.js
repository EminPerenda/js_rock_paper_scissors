...
const updateScoreBoard = () => {
    ...
    cps.ariaLabel = `Computer has ${Game.getCpSession()} wins this session.`;
}
...
const countdownFade = () => {
    ...
    countdown.forEach(e1 => {
        e1.className ="fadeOut";
    })
}
...
const buildActionMessage = (winner,player,computer) =>{
    if(winner === "tie") return "Tie game!";
    if(winner === "computer"){
        const action = getAction(computer);
        return `${properCase(computer)} ${action} ${properCase(player)}.`;
    }
    else {
         const action = getAction(player);
        return `${properCase(player)} ${action} ${properCase(computer)}.`;
    }
}
